 ...
 *
 * In the develop stage RQ checks all the conditions and diplays a warning
 * message if there is one which does not hold. RQ is used usually at
 * the beginnig of functions to check input parameters, where is doing a very
 * similar job as the "pre-conditions" statements in some programming
 ...
 * in macro definition like in functions. Writting more than one
 * pre-condition is than far more comfortable. GCC supports this quite
 * a long time but with different syntax. We try to check the C version
