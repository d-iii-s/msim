 ...
 *
 */
static void write_byte(char **buf, int c)
{
	*((*buf)++) = hexchar[c >> 4];
	*((*buf)++) = hexchar[c & 0xf];
}
...
/** Read a byte in the hex format form *buf and increases the buf pointer
 *
 * Take care for error conditions - end of a string or number corrupt.
 *
 */
static int read_byte(char **buf)
{
	int r;
 ...
 *
 */
static bool gdb_read_mem(uint32_t addr, int length, char *buf)
{
	uint32_t x;
 ...
 *
 */
static bool gdb_write_mem(uint32_t addr, int length, char *buf)
{
	uint32_t val;
 ...
 *
 */
static bool gdb_get_message(char *bufx)
{
...
/** Dump count bytes of memory started from mem do buf in hex
 *
 */
static char *gdb_mem_dump(const char *mem, char *buf, int count)
{
	unsigned char c;
 ...
 *
 */
static char *gdb_mem_upload(const char *buf, char *mem, int count)
{
	unsigned char c;
...
/** Convert internal excepion events to gdb standards
 *
 * This is not used for now.
 *
 */
static int gdb_convert_event(int evno)
{
	switch (evno) {
 ...
 *
 */
static void gdb_cmd_read_mem(char *buf)
{
	char *bufx = buf;
 ...
 *
 */
static void gdb_cmd_write_mem(char *buf)
{
	char *bufx = buf;
	uint32_t addr, length;
 ...
 * 
 */
static void gdb_cmd_step(char *buf, bool cont)
{
	uint32_t addr;
