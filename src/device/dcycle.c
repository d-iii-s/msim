 ...
 *
 */
static void dcycle_read32(r4k_cpu_t *cpu, device_t *dev, ptr36_t addr, uint32_t *val)
{
	ASSERT(dev != NULL);
	ASSERT(val != NULL);
 ...
 *
 */
static void dcycle_read64(r4k_cpu_t *cpu, device_t *dev, ptr36_t addr, uint64_t *val)
{
	ASSERT(dev != NULL);
	ASSERT(val != NULL);
