...
/* makes basic info about instruction */
void
decode_instr( TInstrInfo *ii)

{
	...
	switch (opc->format)
	{
		case ifIMM:
			...
			break;
		case ifJ:
			ii->imm = (ii->icode & TARGET_MASK) << TARGET_SHIFT;
			ii->rt = 0; ii->rs = 0; ii->rd = 0;
			break;
		case ifREG:
			/* registers */
			ii->rs = (ii->icode & RS_MASK) >> RS_SHIFT;
