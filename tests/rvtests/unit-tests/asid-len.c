...
PCUT_TEST(default_value){
    PCUT_ASSERT_INT_EQUALS(rv_asid_len, cpu.csr.asid_len);
}
...
PCUT_TEST(shorter_asid_zeroes_out){
    ...
    cpu.csr.satp = rv_csr_satp_mode_mask | rv_csr_asid_mask | ppn;

    rv_csr_set_asid_len(&cpu, 7);

    unsigned expected_asid = 0x7F;
    
    uint32_t mode_after = cpu.csr.satp & rv_csr_satp_mode_mask;
    unsigned asid_after = (cpu.csr.satp & rv_csr_asid_mask) >> rv_csr_satp_asid_offset;
    uint32_t ppn_after = cpu.csr.satp & rv_csr_satp_ppn_mask;
...
PCUT_TEST(longer_asid_perserves){
    ...
    // set asid to max value of 511 
    cpu.csr.satp = rv_csr_satp_mode_mask | (asid << rv_csr_satp_asid_offset) | ppn;

    rv_csr_set_asid_len(&cpu, 9);
...
PCUT_TEST(csr_write_masks){
    ...
    cpu.regs[instr.i.rs1] = rv_csr_satp_mode_mask | rv_csr_asid_mask;
    cpu.regs[instr.i.rd] = 0;
...
static unsigned probe_asid_len(){
    ...
    cpu.regs[instr.i.rs1] = 0;
    cpu.regs[instr.i.rd] = 0;

    ex = rv_csrrs_instr(&cpu, instr);
    PCUT_ASSERT_INT_EQUALS(rv_exc_none, ex);

    uint32_t satp_content = cpu.regs[instr.i.rd];
...
static void test_asid_len_probe(unsigned asid_len){
    ...
    rv_csr_set_asid_len(&cpu, asid_len);
